...
export class SignupPage implements OnInit {
  ...
  async signUp() {
    ...
    if (this.signup_form.value.email == '' || this.signup_form.value.password == '' || this.signup_form.value.password_repeat == '') {
      this.toastService.presentToast('Error', 'Por favor preencha todos os campos', 'top', 'danger', 4000);

      // If passwords do not match
    } else if (this.signup_form.value.password != this.signup_form.value.password_repeat) {
      this.toastService.presentToast('Error', 'A senha deve ser igual', 'top', 'danger', 4000);
      ...
      this.toastService.presentToast('Bemvindo!', 'Lorem ipsum', 'top', 'success', 2000);
